...
int main(){
    ...
    while(true){
        // Grab user input
        symbol = (t % 2) ? 'O' : 'X';
        cout << "Please enter co-ordinate for '" << symbol << "'" << endl;
        cout << "x: ";
        cin >> x_pos;
        cout << "y: ";
        cin >> y_pos;
        ...
        if ((0 < x_pos && x_pos <= m_size) && (0 < y_pos && y_pos <= n_size)){
            if ((array[x_pos-1][y_pos-1] != 'X') && (array[x_pos-1][y_pos-1] != 'O')){
                array[x_pos-1][y_pos-1] = symbol;
            }
            else{
                cout << "Co-ordinate (" << x_pos << "," << y_pos << ") already taken! Try again." << endl;
                continue;
            }
        }
        else{
